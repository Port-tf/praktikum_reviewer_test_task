...
# В них хранить название валюты и константу, если константа не предусмотренна, храним None.
# Тогда от условий ниже можно будет избавиться, оставить проверку только на то, что для валюты есть
# константа, cash_remained делим на константу, и тут же можно применить округление. При таком
# подходе сколько бы валют у нас в программе не использовалось, достаточно будет добавить
# в словарь пару для этой валюты, а не дописавать новое условие.
# Нужно будет еще предусмотреть случай, если пользователь передаст ключ, которого нет в словаре,
# чтобы код не сломался, а возвращено сообщение о .
        currency_type = currency
        cash_remained = self.limit - self.get_today_stats()
# Если будет словарь, эту строчку можно убрать.
        if currency == 'usd':
# Если будет словарь, эту строчку универсально будем использовать для любой валюты,
# ватаскивая константы из кортежа.
            cash_remained /= USD_RATE
# Если будет словарь, эту строчку можно убрать.
            currency_type = 'USD'
# Выше для условия проверки пришедшей строки использовалась переменная currency,
# здесь тогда логично использовать ее же.
# Если будет словарь, этот блок проверки не понадобится.
...
# и ее передаем. Переделаем на словарь с кортежем, и округление можно сделать выше при расчетах,
